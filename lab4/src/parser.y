...
FuncDef
    : BType ID LPAREN {
        ...
        $$ = new FuncDef(globals->lookup($2), $7);
        typeStack.pop_back();
    ...
    | BType ID LPAREN {
        ...
        $$ = new FuncDef(globals->lookup($2), $8);
        typeStack.pop_back();
        SymbolTable *top = identifiers;
        identifiers = identifiers->getPrev();
        delete top;
...
LOrExp
    : LAndExp {
        ...
        if (exprSE->getType()->isConst())
            se = new TemporarySymbolEntry(TypeSystem::constBoolType, SymbolTable::getLabel(), exprSE->getIntValue() != 0 ? 1 : 0, 0.0);
        else
